...
class AlexDataNode : public AlexNode<T, P> {
 public:
  ...
  inline P& get_payload(const T& key) const {
    P values_ = at(key,false);
    return values_;
  }
  ...
  struct lipp_Node;
  lipp_Node* root;
  inline int PREDICT_POS(lipp_Node* node,const T& key) const {
        if(node==nullptr)
            return 0;
        int tr = node->tree_level;
        if(tr==0){
            double vd=this->model_.predict_double(key);
            if (vd > std::numeric_limits<int>::max() / 2) {
               return node->num_items - 1;
            }
            if (vd < 0) {
               return 0;
            }
            return std::min(node->num_items - 1, static_cast<int>(vd));
        }
        else{
  ...
  bool lipp_insert(const V& v) {
      return lipp_insert(v.first, v.second);
  }

  bool lipp_insert(const T& key, const P& value) {
      bool ok = true;
      root = lipp_insert_tree(root, key, value, &ok);
      key_comparison(key);
      return ok;
  }

  P at(const T& key, bool skip_existence_check) const {
        lipp_Node* node = root;
        int pos_;

        while (true) {
            pos_ = PREDICT_POS(node, key);
            if (BITMAP_GET(node->child_bitmap, pos_) == 1) {
                node = node->items[pos_].comp.child;
            } else {
                if (skip_existence_check) {
                    return node->items[pos_].comp.data.value;
                } else {
                    if (BITMAP_GET(node->none_bitmap, pos_) == 1) {
                        return static_cast<P>(0);
          
                    } else if (BITMAP_GET(node->child_bitmap, pos_) == 0) {
                        return node->items[pos_].comp.data.value;
                    }
                }
            }
    ...
    bool remove(const T &key) {
        ...
        lipp_Node *path[MAX_DEPTH];
        int path_size = 0;
        lipp_Node *parent = nullptr;

        for (lipp_Node* node = root; ; ) {
            path[path_size++] = node;
            // node->size--;
  ...
  void lipp_bulk_load(const V* vs, int num_keys) {
        if (num_keys == 0) {
            destroy_tree(root);
            root->tree_level = 0;
            root = build_tree_none();
    ...
    std::stack<lipp_Node*> pending_two;

    std::allocator<lipp_Node> node_allocator;

    lipp_Node* new_nodes(int n)
    {
        lipp_Node* p = node_allocator.allocate(n);
        return p;
    }
    void delete_nodes(lipp_Node* p, int n)
    {
    ...
    std::allocator<bitmap_t> bitmap_allocator_;
    bitmap_t* new_bitmap(int n)
    {
        bitmap_t* p = bitmap_allocator_.allocate(n);
        return p;
    }
    void delete_bitmap(bitmap_t* p, int n)
    {
        bitmap_allocator_.deallocate(p, n);
    }


  lipp_Node* build_tree_none()
    {
        lipp_Node* node = new_nodes(1);
        node->is_two = 0;
        node->build_size = 0;
        node->size = 0;
        node->fixed = 0;
    ...
    lipp_Node* build_tree_two(T key1, P value1, T key2, P value2)
    {
        key_comparison(key1);
        key_comparison(key2);
        if (key1 > key2) {
    ...
    lipp_Node* build_tree_bulk(T* _keys, P* _values, int _size)
    {
        for(int i=0;i<_size;i++)
            key_comparison(_keys[i]);
        if (1) {
    ...
    lipp_Node* build_tree_bulk_fast(T* _keys, P* _values, int _size)
    {
        ...
        while (!s.empty()) {
            ...
            lipp_Node* node = s.top().node;
            s.pop();

            if (end - begin == 2) {
                lipp_Node* _ = build_tree_two(_keys[begin], _values[begin], _keys[begin+1], _values[begin+1]);
                memcpy(node, _, sizeof(lipp_Node));
                delete_nodes(_, 1);
            } else {
                T* keys = _keys + begin;
                ...
                for (int item_i = PREDICT_POS(node, keys[0]), offset = 0; offset < size; ) {
                    ...
                    if (next == offset + 1) {
                        ...
                        node->items[item_i].comp.child->tree_level = node->tree_level + 1;
    ...
    /// bulk build, _keys must be sorted in asc order.
    /// FMCD method.
    lipp_Node* build_tree_bulk_fmcd(T* _keys, P* _values, int _size)
    {
    ...
    void destroy_tree(lipp_Node* lipp_root)
    {
        if(lipp_root==nullptr)
          return;
        std::stack<lipp_Node*> s;
        s.push(lipp_root);
        while (!s.empty()) {
            lipp_Node* node = s.top(); s.pop();
    ...
    lipp_Node* lipp_insert_tree(lipp_Node* _node, const T& key, const P& value, bool* ok = nullptr)
    {
        constexpr int MAX_DEPTH = 128;
        lipp_Node* path[MAX_DEPTH];
        int path_size = 0;
        ...
        for (lipp_Node* node = _node; ; ) {
            ...
            if (BITMAP_GET(node->none_bitmap, pos) == 1) {
                ...
                node->items[pos].comp.child->tree_level = node->tree_level + 1;
                insert_to_data = 1;
                break;
            } else {
                node = node->items[pos].comp.child;
        ...
        for (int i = 0; i < path_size; i ++) {
            ...
            if (need_rebuild) {
                ...
                #endif
                lipp_Node* new_node = build_tree_bulk(keys, values, ESIZE);
                #if COLLECT_TIME
  ...
  void bulk_load(const V values[], int num_keys,
                 ...
                 bool train_with_sample = false) {
                  if(values==nullptr)
                      return;
                  lipp_bulk_load(values,num_keys);
                 }
